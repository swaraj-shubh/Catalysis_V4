...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 lg:px-12 transition-all duration-300">
          ...
          <div className={`hidden md:flex items-center gap-1 lg:gap-3 px-4 py-2 rounded-2xl transition-all duration-300 ${
            isScrolled ? "bg-white/50 border-2 border-black/10 backdrop-blur-sm" : ""
          }`}>
            {navItems.map((item) => {
          ...
          <div className="hidden md:flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 bg-[#DD273E] text-white font-black text-sm lg:text-base px-6 py-2.5 rounded-xl border-2 border-black shadow-[4px_4px_0px_black] hover:-translate-y-1 hover:shadow-[6px_6px_0px_black] active:translate-y-0.5 transition-all"
            >
              REGISTER NOW
          ...
          {/* Hamburger */}
          <button
            className={`md:hidden flex flex-col justify-center items-center w-12 h-12 z-[110] rounded-xl border-[3px] border-black shadow-[4px_4px_0px_black] active:shadow-[2px_2px_0px_black] active:translate-x-[2px] active:translate-y-[2px] transition-all ${
              isOpen ? "bg-[#FFEEF0]" : "bg-white"
            }`}
      ...
      {/* MOBILE PANEL */}
      <div
        className={`md:hidden fixed top-0 right-0 h-full w-[85%] max-w-[340px] z-[108] bg-[#FFEEF0] border-l-4 border-black shadow-[-15px_0px_40px_rgba(0,0,0,0.3)] transition-transform duration-500 overflow-y-auto ${
              ...
              return (
                ...
                >
                  <span className={`transition-colors ${
                    isActive ? "text-[#DD273E]" : "text-[#3A001D] group-hover:text-[#DD273E]"
                  }`}>
                    {item.label}
                  </span>
